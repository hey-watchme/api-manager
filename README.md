...
具体的なデプロイスクリプト (`deploy-ecr.sh`, `run-prod.sh` など) は、リポジトリ内の各ファイルを参照してください。
...
特に、以下の点については、`watchme-server-configs` のドキュメントで詳細な解説とテンプレートを提供しています。

- Nginxの `location` と `proxy_pass` の関係性
- コンテナ間通信のルール (例: `http://api-transcriber:8001`)
- トラブルシューティングの手法
