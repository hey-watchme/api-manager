        ...
        <div className="flex items-center">
          ...
          <div>
            <h4 className={`font-medium ${isSuccess ? 'text-green-900' : 'text-red-900'}`}>
              {isSuccess ? 'Azure処理完了' : 'Azure処理失敗'}
            </h4>
            <p className={`text-sm ${isSuccess ? 'text-green-800' : 'text-red-800'}`}>
              {message || (isSuccess ? 'Azure Speech Serviceで正常に処理されました' : 'Azureでエラーが発生しました')}
            </p>
          </div>
        </div>
      </div>
        ...
        {isSuccess && (
          <div className="bg-blue-50 border border-blue-200 rounded p-3">
            <p className="text-xs text-blue-800">
              <span className="font-medium">💡 Azure Speech Service:</span>
              処理が完了しましたが、本番環境でAzureアクセスの問題により認識結果が空の場合があります。
              データベースの vibe_whisper テーブルを確認してください。
            </p>
          </div>
        )}
